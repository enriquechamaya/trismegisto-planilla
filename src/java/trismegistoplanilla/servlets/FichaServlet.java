...
//    }
                ...
                if (JOvalidarExperienciaLaboral.getBoolean("status")) {
                  ...
                  if (JOvalidarRegimenPensionario.getBoolean("status")) {
                    ...
                    if (jsonObjRegistrarFicha.getBoolean("status")) {
                      ...
                      c.setAsunto("ENHORABUENA!, SU REGISTRO HA SIDO COMPLETADO CON ÉXITO");
                    ...
                    out.print(JOvalidarRegimenPensionario);
                  }
                } else {
                  out.print(JOvalidarExperienciaLaboral);
                }
              } else {
                out.print(JOvalidarFormacionAcademica);
              }
            } else {
              out.print(JOvalidarDatosFamiliares);
            }
          } else {
            out.print(JOvalidarDatosPersonales);
          }
  ...
  private JSONObject validarFormacionAcademica(JSONObject json, PersonaBean p, HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    for (int j = 0; j < longitudFormacionAcademica; j++) {
      ...
      if (formacionAcademica.getInt("codigoNivelEstado") == 0) {
        JOResponse.put("status", false);
      ...
      // sector institucion
      if (formacionAcademica.isNull("sectorInstitucion")) {
        JOResponse.put("status", false);
        JOResponse.put("message", "El parámetro [FORMACION ACADÉMICA - SECTOR INSTITUCION] es nulo");
        return JOResponse;
      }

      if (!ParamsValidation.validaSoloLetras(formacionAcademica.getString("sectorInstitucion"))) {
        JOResponse.put("status", false);
        JOResponse.put("message", "El parámetro [FORMACION ACADÉMICA - SECTOR INSTITUCION] solo acepta letras");
        return JOResponse;
      }
